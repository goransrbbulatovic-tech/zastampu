...
class JsonDB {
  constructor(filePath) {
    ...
    this.data = { transactions: [], nextId: 1 };
    this._load();
  }
  _load() {
    try {
...
function validateLicense(key) {
  return key.toUpperCase().trim() === generateExpectedKey(getMachineId());
}
...
app.whenReady().then(() => {
  initDatabase();
  licenseValid = checkStoredLicense();
  createWindow();
});
...
ipcMain.handle('get-pdf-data', () => {
  try { return { transactions: db.getTransactions({}), stats: db.getStats().total }; }
  catch(e) { return null; }
});
