...
async function loadDashboard() {
  ...
  const mProfit = thisMonth.zarada - thisMonth.trosak;
  document.getElementById('kpi-profit-month').textContent = 'Ovaj mj.: ' + fmt(mProfit);
...
async function quickAdd() {
  ...
  if (result.success) {
    ...
    await loadDashboard();
    showToast(
      `${currentType === 'zarada' ? '💰 Zarada' : '💸 Trošak'}: ${fmt(amount)} dodano!`,
      currentType === 'zarada' ? 'success' : 'error'
    );
  } else {
...
async function loadStatistics() {
  ...
  // Profit chart
  const profitData = byMonth.map(m => m.zarada - m.trosak);
...
async function exportExcel() {
  ...
  if (result.success) {
    showMsg(msg, '✓ Excel fajl sačuvan!', 'success');
    showToast('Excel izvještaj uspješno izvezen!', 'success');
  } else {
    showMsg(msg, result.message || 'Greška pri izvozu.', 'error');
  }
}
...
async function exportPdf() {
  ...
  try {
    ...
    doc.text('Izvještaj generisan: ' + new Date().toLocaleDateString('bs-BA'), 20, 30);
    ...
    doc.text(data.stats.totalZarada.toFixed(2) + ' KM', 20, 65);
    doc.setTextColor(231, 76, 60);
    doc.text(data.stats.totalTrosak.toFixed(2) + ' KM', 65, 65);
    doc.setTextColor(201, 168, 76);
    doc.text(profit.toFixed(2) + ' KM', 110, 65);
    doc.setTextColor(41, 128, 185);
    doc.text(margin + '%', 155, 65);
    ...
    doc.autoTable({
      startY: 97,
      head: [['#', 'Tip', 'Iznos (KM)', 'Kategorija', 'Opis', 'Datum']],
      body: data.transactions.map(t => [
        t.id,
        t.type === 'zarada' ? 'Zarada' : 'Trošak',
    ...
    const result = await window.api.savePdf(new Uint8Array(pdfArrayBuffer));
    if (result.success) {
      showMsg(msg, '✓ PDF sačuvan!', 'success');
      showToast('PDF izvještaj uspješno kreiran!', 'success');
    } else {
      showMsg(msg, 'Greška pri čuvanju.', 'error');
    }
  } catch (e) {
    showMsg(msg, 'Greška: ' + e.message, 'error');
  }
}

// ── HELPERS ───────────────────────────────────────────────────
function fmt(amount) {
  return Number(amount).toFixed(2).replace('.', ',') + ' KM';
}
